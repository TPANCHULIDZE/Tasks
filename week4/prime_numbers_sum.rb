...
class Numbers
  ...
  def check_validate(number)
    unless is_number_positive_integer?(number)
      raise IntegerError, "Input value have to be positive integer"
    end
  end
...
puts PrimeNumbersSumCounter.new(numbers).call
