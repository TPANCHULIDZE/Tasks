...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        invalid_inputs = ['dd', 'dssd']
        valid_input = '6'
        allow(game_input).to receive(:gets).and_return(invalid_inputs.first, invalid_inputs.last, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    subject(:game) { described_class.new(1, 13) }
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game.instance_variable_get(:@maximum)
        input = 20
        expect(game.verify_input(min, max, input)).to be_nil
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      ...
      it 'returns 9' do
        expect(game.maximum_guesses).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game.display_turn_order(search_display) }.to change { game.instance_variable_get(:@guess_count)}.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_display).to receive(:make_guess).once
      game.display_turn_order(search_display)
    end
